//
// Created by Orange on 2023/11/6.
//
#include <graphics.h>
#include <conio.h>
#include <time.h>
...
void init_board(Board& board) {
  ...
  board.score = 0;
  board.is_over = false;
  loadimage(&board.bg_image, "res/bk.jpg", CELL_SIZE * BOARD_WIDTH, CELL_SIZE * BOARD_HEIGHT);
  loadimage(&board.food_image, "res/block0.png", CELL_SIZE, CELL_SIZE);
...
void draw_board(const Board& board) {
  // 绘制背景图片
  putimage(0, 0, &board.bg_image);

  // 绘制食物
  if (!board.is_over) {
    putimage(board.food_x * CELL_SIZE, board.food_y * CELL_SIZE, &board.food_image);
  }
